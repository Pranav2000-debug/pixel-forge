...
const VideoCard: React.FC<VideoCardProps> = ({ video, onDownload, onDelete }) => {
  ...
  const compressionPercentage: number = Math.round((1 - Number(video.compressedSize) / Number(video.originalSize)) * 100);
    ...
    <div
      ...
      <figure className="aspect-video relative">
        {isHovered ? (
          previewError ? (
            ...
            <video src={getPreviewVideoUrl(video.publicId)} autoPlay muted loop className="w-full h-full object-cover" onError={handlePreviewError} />
          )
        ) : (
          <img src={getThumbnailUrl(video.publicId)} alt={video.title} className="w-full h-full object-cover" />
        )}
      ...
      <div className="card-body p-4">
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <button className="btn btn-primary btn-sm" onClick={() => onDelete(video.publicId)}>
            Delete
          </button>
        </div>
      </div>

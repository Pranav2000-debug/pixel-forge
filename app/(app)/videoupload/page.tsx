...
function VideoUpload() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // setting up the formdata that our api requires
    ...
    try {
      const res = await axios.post("api/video-upload", formData, { headers: { "Content-Type": "multipart/form-data" } });
      if (res.status !== 200) throw new Error("Failed to upload video to cloudinary");
    } catch (error) {
      console.log(error);
    } finally {
